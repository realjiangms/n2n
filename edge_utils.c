...
int run_edge_loop(n2n_edge_t * eee, int *keep_running) {
  ...
  *keep_running = 1;
  ...
  while(*keep_running) {
    ...
    numPurged =  purge_expired_registrations(&(eee->known_peers), &last_purge_known, 0, 0);
    numPurged += purge_expired_registrations(&(eee->pending_peers), &last_purge_pending, 0, 0);
